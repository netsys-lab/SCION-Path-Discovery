...
func Test_SCIONSocket(t *testing.T) {
	t.Run("SCIONSocket Listen", func(t *testing.T) {
		sock := NewSCIONSocket("19-ffaa:1:cf1,[127.0.0.1]:21000")
		err := sock.Listen()
		if err != nil {
			t.Error(err)
			return
	...
	t.Run("SCIONSocket Listen And Dial", func(t *testing.T) {
		...
		sock2 := NewSCIONSocket("19-ffaa:1:cf1,[127.0.0.1]:11000")
		err = sock2.Listen()
		if err != nil {
			t.Error(err)
			return
		...
		go func() {
			paths, err := lookup.PathLookup("19-ffaa:1:cf1,[127.0.0.1]:21000")
			if err != nil {
				t.Error(err)
				return
			}
